...
public class JobServiceTests
{
    ...
    public async Task CreateJob_ReturnsDto_WithCorrectClientId()
    {
        ...
        var dto = new CreateJobDto("Fix sink", "Leaking pipe", JobCategory.Plumbing, "Bucharest", "RO", null, null, null, null, null, null, null);
    ...
    public async Task GetJobById_ReturnsDto_WhenExists()
    {
        var (ctx, svc) = Build();
        var userId = SeedProfile(ctx).Id;
        var created = await svc.CreateJobAsync(userId, new CreateJobDto("Tile work", "Bathroom", JobCategory.General, "Cluj", "RO", null, null, null, null, null, null, null));
    ...
    public async Task UpdateJobStatus_ThrowsForbidden_WhenNotOwner()
    {
        ...
        var job = await svc.CreateJobAsync(ownerId, new CreateJobDto("Job", "Desc", JobCategory.General, "City", "RO", null, null, null, null, null, null, null));
    ...
    public async Task UpdateJobStatus_UpdatesStatus_WhenOwner()
    {
        var (ctx, svc) = Build();
        var userId = SeedProfile(ctx).Id;
        var job = await svc.CreateJobAsync(userId, new CreateJobDto("Job", "Desc", JobCategory.General, "City", "RO", null, null, null, null, null, null, null));
    ...
    public async Task GetJobs_FiltersByCategory()
    {
        var (ctx, svc) = Build();
        var userId = SeedProfile(ctx).Id;
        await svc.CreateJobAsync(userId, new CreateJobDto("A", "D", JobCategory.Plumbing, "City", "RO", null, null, null, null, null, null, null));
        await svc.CreateJobAsync(userId, new CreateJobDto("B", "D", JobCategory.Electrical, "City", "RO", null, null, null, null, null, null, null));
